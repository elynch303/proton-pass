...
- Session lock/unlock happens inline in the popup where possible; login
  always opens a floating terminal, since it's a one-time interactive
  web-login/2FA flow
...
reimplement idle detection, it just surfaces `pass-cli session lock/unlock`
in the UI.
