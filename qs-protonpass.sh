...
# hang for anywhere near N_vaults * per-vault-timeout — computed fresh
# inside fetch() (not once before dispatch) so a vault whose turn comes
# late because every worker slot (max_workers, below) was already busy
# sees its actual remaining budget at the moment it starts, not the full
